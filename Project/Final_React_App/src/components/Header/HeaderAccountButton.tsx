...
const {isSuccess, isError, data } = useQuery({
    ...
    return (
        <>
            {isSuccess && <Link to={`/profile/${x}`} className='header-nav__link'><button className='btn-reset header-btn'>{x}{isMobile && <SvgUserLogined />}</button></Link>}
            {isError && <button className='btn-reset header-btn' onClick={isOpen}>Войти{isMobile && <SvgUser />}</button>}
        </>
    )
}
