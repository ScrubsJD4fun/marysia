...
export const PromoButtons: FC<ShowButtons> = ({
  showAllButtons,
  onRefresh,
  id,
  trailer,
  ...
  const handleFavorites = () => {
    userIsLogined ? favoriteMutate.mutate() : isOpen()
  }

  return (
    <div className="promo-buttons">
      <button onClick={isOpenTrailer} className="promo-trailer">
        Трейлер
      </button>
      {trailerHandler && (
        <Modal
          children={<Trailer videoId={trailer as string} />}
          closeModal={isCloseTrailer}
        />
      )}
      {showAllButtons && (
      ...
      {showAllButtons && (
        <button onClick={handleRefresh} className="promo-refresh"></button>
      )}
    </div>
  )
}
